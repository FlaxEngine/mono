...
#if G_HAVE_API_SUPPORT(HAVE_UWP_WINAPI_SUPPORT)
		result = CreateFileMappingFromApp((HANDLE)handle, NULL, get_page_access (access) | options, (ULONG64)*capacity, mapName);
#else
		result = CreateFileMappingW((HANDLE)handle, NULL, get_page_access(access) | options, (DWORD)(((guint64)*capacity) >> 32), (DWORD)*capacity, mapName);
#endif
...
mono_mmap_open_file (const gunichar2 *path, gint path_length, int mode, const gunichar2 *mapName, gint mapName_length, gint64 *capacity, int access, int options, int *ioerror, MonoError *error)
{
	...
	if (path) {
		...
		hFile = CreateFile2 (path, get_file_access (access), FILE_SHARE_READ, mode, &param);
#else
